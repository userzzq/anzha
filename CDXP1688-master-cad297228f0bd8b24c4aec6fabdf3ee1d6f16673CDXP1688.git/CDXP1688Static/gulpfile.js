...
function jslibBuild(config) {
  ...
  libs.push(baseDir + appconfig.common.libhuhuiyu + 'angularjs.utils-1.0.2.min.js');
...
function htmlBuild(config) {
  ...
  gulp.src([baseDir + config.html + 'index.html', baseDir + config.html + 'pay.html', baseDir + config.html + 'wxpay.html']).pipe(gulp.dest(baseDir + config.dist));
  if (!includeTest) {
...
/*manage目录相关==========================================================================*/
/*清理发布*/
gulp.task('manage-clear', function() {
  return clearBuild(appconfig.manage);
});

/*项目lib文件打包======================================================================*/
/*js第三方库打包*/
gulp.task('manage-jslib', function() {
  return jslibBuild(appconfig.manage);
});

/*css第三方库*/
gulp.task('manage-csslib', function() {
  return csslibBuild(appconfig.manage);
});

/*字体文件*/
gulp.task('manage-fonts', function() {
  return fontsBuild(appconfig.manage);
});

gulp.task('manage-lib', ['manage-jslib', 'manage-csslib', 'manage-fonts'], function() {
  console.log('处理第三库依赖');
});

/*项目相关文件处理======================================================================*/
/*图片文件处理*/
gulp.task('manage-images', function() {
  return imagesBuild(appconfig.manage);
});

/*项目html文件*/
gulp.task('manage-html', function() {
  return htmlBuild(appconfig.manage);
});

/*项目js文件*/
gulp.task('manage-js', function() {
  return jsBuild(appconfig.manage);
});

/*项目css打包*/
gulp.task('manage-css', function() {
  return cssBuild(appconfig.manage);
});

gulp.task('manage-dev', ['manage-images', 'manage-html', 'manage-js', 'manage-css'], function() {
  console.log('处理开发任务');
});
/*manage目录相关结束==========================================================================*/
...
/*默认打包任务*/
gulp.task('default', ['exclude-test', 'app-clear', 'app-lib', 'app-dev', 'manage-clear', 'manage-lib', 'manage-dev'], function() {
  del.sync(baseDir + appconfig.app.distjs + '*.map');
  del.sync(baseDir + appconfig.manage.distjs + '*.map');
  console.log('打包任务完成');
});
...
/*manage开发watch任务*/
gulp.task('manage-watch', ['manage-lib', 'manage-dev'], function() {
  watch([baseDir + appconfig.manage.js + '**/*.js'], function() {
    gulp.start('manage-js');
  });

  watch([baseDir + appconfig.manage.css + '**/*.css'], function() {
    gulp.start('manage-css');
  });

  watch([baseDir + appconfig.manage.html + '**/*.html'], function() {
    gulp.start('manage-html');
  });

  watch([baseDir + appconfig.manage.images + '**/*'], function() {
    gulp.start('manage-images');
  });
});
