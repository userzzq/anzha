(function() {
  ...
  ctrls.controller('LiKeXiaDanCtrl', ['$scope', '$log', '$timeout', 'UtilsService', 'MyDataService', 'DialogService', LiKeXiaDanCtrl]);

  function LiKeXiaDanCtrl($scope, $log, $timeout, UtilsService, MyDataService, DialogService) {
    $log.debug('LiKeXiaDanCtrl init...');

    // 处理scope销毁
    $scope.$on('$destroy', function() {
      $log.debug('LiKeXiaDanCtrl destroy...');
    });
    ...
    function showImg(file) {
      $log.debug('=========>', file, file.size, MyAppConfig.filelimit);
      if (file.size > MyAppConfig.filelimit) {
        DialogService.showAlert('图片大小超过限制', function() {
          fileEle = null;
          $scope.upimg = 'images/sctp.png';
        });
        return;
